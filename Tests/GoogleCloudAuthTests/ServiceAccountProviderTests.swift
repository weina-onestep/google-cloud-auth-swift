...
@Suite struct ServiceAccountProviderTests {
    ...
    )) func createSessionProduction() async throws
  {
    ...
    let provider = try #require(
      try ServiceAccountProvider(environmentVariableName: environmentVariableName))
  ...
  @Test func createSessionShouldHandleMultipleRequests() async throws {
    let (eventLoopGroup, tokenURI, mockedTokenServerChannel) =
      try await self.mockServiceAccountTokenServer()
  ...
  private func createMockCredentials(tokenURI: String) -> ServiceAccountProvider.Credentials {
    ServiceAccountProvider.Credentials(
      ...
      clientX509CertURL:
        "https://www.googleapis.com/robot/v1/metadata/x509/mock-service-account%40mock-project-id.iam.gserviceaccount.com"
    )
  }
